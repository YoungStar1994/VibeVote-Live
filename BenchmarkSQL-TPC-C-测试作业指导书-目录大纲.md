...
- 验证 Java 运行环境：`java -version`、`javac -version`。
- 构建示例：`./gradlew clean build` 或 `mvn -T 4C -DskipTests package`。
...
### 2.7 预检查清单（Pre-flight Checklist）
- 版本冻结：BenchmarkSQL/驱动/数据库版本记录。
- 访问连通：压测机到数据库 `telnet/ss`/白名单验证。
- 账号权限：建表/索引/加载/事务所需权限具备。
- 资源基线：空闲 CPU/内存/IO/网络基线采集 5 分钟。
- 时钟同步：`chronyc tracking` 或 `ntpq -p` 正常。
- 安全合规：数据隔离、脱敏、清理策略确认。
...
### 3.4 测试计划模板与评审
- 场景矩阵（仓数 × 并发 × 时长 × 事务权重）。
- 通过/失败判定标准（吞吐、延迟、错误率、资源利用率）。
- 变更控制：一次只改变一个变量，留存配置版本。
...
#### 4.1.1 端到端命令示例（PostgreSQL）
- 创建数据库与用户：`createdb tpcc; createuser tpcc` 或等价 SQL。
- props 样例关键项：`driver=org.postgresql.Driver`、`conn=jdbc:postgresql://host:5432/tpcc`。
- 初始化：`./run/create-schema.sh props.pg`
- 装载：`./run/load-data.sh props.pg`
- 压测：`./run/runBenchmark.sh props.pg`
...
- 命令示例：
  - `sar -n DEV 1 300`、`iostat -xmd 1 300`、`vmstat 1 300`、`mpstat -P ALL 1 300`。
  - `pidstat -hlrut 1 120 -p <java_pid>`。
...
### 8.5 可复现性、版本化与日志规范
- Git 版本化：记录 BenchmarkSQL、驱动、配置、脚本的提交哈希。
- 配置冻结：`props` 文件与 SQL 模板纳入版本库，打标签发布。
- 审计日志：保留控制台输出、错误日志、GC 日志、数据库慢日志。
- 元数据记录：测试编号、开始/结束时间、仓数/并发、环境与主机信息。
...
### 9.1 安全、回滚与风险缓解
- 安全：在隔离环境执行、使用最小权限账号、清理残留数据。
- 回滚：变更前备份配置与元数据，提供一键回滚脚本（删表/撤销索引）。
- 风险：设置负载上限、超时阈值与自动停止条件（错误率、延迟）。

## 10. FAQ 与故障排除
- 常见报错清单与快速定位步骤、典型修复措施。
- 性能回退场景与对比方法、回归与复现策略。
- 速查表：连接失败/权限错误/锁等待/超时/驱动不匹配。

## 11. 报告与交付物
- 报告模板：摘要、环境、方法、结果、分析、建议、附录。
- 图表清单：TPS/延迟曲线、资源利用率、错误率、配置一览。
- 交付物：压测配置、脚本与日志、监控导出、数据校验结果。
